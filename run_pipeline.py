...
def main() -> None:
    ...
    def step_postprocess():
        """Re-run final prose formatting on the active job deliverable in place."""
    ...
    def step_prepare_rotated():
        ...
        print(f"Prepare rotated: {len(prepared)} table page(s) → {rot_for}/")
    ...
    def write_docs():
        ...
        report_path.write_text(
            ...
            f"(`{work_rel}/rotated_for_grok/` + `rotated_from_grok/`; see "
...
1. Parse `{out_rel}/db_import_registry.json` into `artifacts` + `artifact_product_tiers`.
2. Parse `<!-- db:... -->` comments in `{ctx.markdown_name}` for prose blocks → `content_nodes`.
3. Split pipe tables under each artifact into `scale_rows`.
4. Copy `{out_rel}/assets/` paths into `figure_assets`; optionally store BLOB for portability.
...
""",
        ...
        print(f"Wrote documentation under {meta}")
