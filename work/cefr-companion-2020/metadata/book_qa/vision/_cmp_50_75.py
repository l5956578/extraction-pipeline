"""Compare PDF text vs MD slices for pages 50-75 (book vision QA helpers)."""
...
ROOT = Path(__file__).resolve().parents[4]  # extraction-pipeline
# work/cefr-companion-2020/metadata/book_qa/vision -> parents[4] = extraction-pipeline
# actually: vision=0, book_qa=1, metadata=2, cefr=3, work=4, root=5
...
def main() -> None:
    ...
    for n in range(50, 76):
        body = bodies[n]
        # also check neighboring MD pages for multipage-table content
        ...
        # dual-emit soup after ```text fence
        soup = []
        for m in re.finditer(r"```\s*\n", body):
            after = body[m.end() : m.end() + 300]
            # if next non-empty line is not heading/el comment and looks like leaf
        ...
        print(f"=== p{n} miss={miss_pct:.0f}% miss_neigh={miss_n_pct:.0f}% "
              f"miss_ph={len(miss_ph)} table={has_pipe_table} fig={has_figure} "
              f"bq={has_bq} soup={len(soup)} cap={captions[:1]}")
        if miss_ph[:3]:
            print("  orphan phrases (not on n-1..n+1):")
            for p in miss_ph[:3]:
                print("   -", p)
        if soup:
            print("  soup:", soup)
        if miss_n_pct >= 30:
            print("  missing words sample:", missing_n[:12])
