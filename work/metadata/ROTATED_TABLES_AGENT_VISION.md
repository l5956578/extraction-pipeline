...
| Role | Location |
|------|----------|
| PNG + JSON + handoff | `work/metadata/rotated_for_grok/page_{NNN}_{span_group_id}.*` |
| Manifest | `work/metadata/rotated_for_grok/manifest.json` |
| Vision output | `work/metadata/rotated_from_grok/page_{NNN}_{span_group_id}.md` |
...
2. For each pending slug in the manifest:
   - Open `work/metadata/rotated_for_grok/{slug}.png` with vision.
   ...
   - Write **only** the table markdown to `work/metadata/rotated_from_grok/{slug}.md`.
